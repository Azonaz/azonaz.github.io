---
layout: default
title: Greek Words — learning Greek apps 
published: true
permalink: /greek-words-app/
last_modified_at: 2024-05-19
---
# Greek Words

These are three small applications for learning Greek words used in courses for A1, A2 and B1 levels.

### Features

- practice words for a chosen topic (meeting people, family etc.),
- check your knowledge for random words from all topics,
- solve a puzzle — guess the word from given letters,
- learn how verbs change across different tenses.

[Download A1 in App Store](https://apps.apple.com/cy/app/greek-words-a1/id6474042509)  
[Download A2 in App Store](https://apps.apple.com/cy/app/greek-words-a2/id6736978135)  
[Download B1 in App Atore](https://apps.apple.com/cy/app/greek-words-b1/id6754924042)

## Privacy Policy
_Last updated: November 5th, 2025_

This Privacy Policy applies to all versions of the “Greek Words” apps, including A1, A2, and B1 levels.

We (developers and publishers)  
are providing this Privacy Policy  
to explain our practices regarding the collection and use of information within the app.

### Collection and use information

Greek Words apps do not collect any personally identifiable information.

Only the Greek Words A1 app uses a third-party service ([Google AdMob](https://policies.google.com/technologies/partner-sites))
to serve ads and collect anonymous usage data.
Please review the privacy policy of this third-party service for more information regarding how they handle personal data.

The Greek Words A2 and Greek Words B1 apps do not use any third-party services that collect or share user data.

### Revisions to this Privacy Policy

We may update our Privacy Policy from time to time.
You are advised to review this page periodically for any changes.
These changes are effective immediately upon posting on this page.

